...
class FeatureMaskExtractor(BaseFeaturesExtractor):
    ...
    def forward(self, x):
        ...
        # reshape obs to (batch_size, 64, 5)
        obs = obs.view(-1, 64, 5)
...
class ShobuPolicy(ActorCriticPolicy):
    def __init__(self, observation_space, action_space, lr_schedule, *args, **kwargs):
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        super(ShobuPolicy, self).__init__(
            observation_space,
            action_space,
            lr_schedule,
            *args,
...
if __name__ == "__main__":
    ...
    print(f"Obs: {obs}")
    policy = ShobuPolicy(gym.spaces.Box(low=0, high=1, shape=(OBS_LENGTH + ACTIONS,)), gym.spaces.Discrete(ACTIONS), lr_schedule=lambda x: 1e-4)
